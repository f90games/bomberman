...
var TimerTask = function(method, scope, config){
...
TimerTask.prototype = {
  ...
  count: function(){
    ...
    if (this._frame == 0){
      if (!this.once)
        this._frame = this.frames;

      this.method.call(this.scope);
    } 
  },
...
Timer.prototype = {
  ...
  tick: function(){
    var timer = this;
    _.each(timer._eventheap, function(task, key){
      task.count();
      if (task.isDone()){
        timer.kill(key);
      }
  ...
  _runTask: function(method, scope, timeout, once){
    var frames = timeout / this.gameFrameTime;
    var task = new TimerTask(method, scope, {"frames": frames, once: once});

    this._eventheap[task.taskId] = task;

    // console.log('Running ' + task.taskId + (once ? ' once' : ' in loop'));

    return task;    
  },

  run: function(method, scope, timeout){
    return this._runTask(method, scope, timeout, false);
  },

  runOnce: function(method, scope, timeout){
    return this._runTask(method, scope, timeout, true);
  },

  kill: function(taskId){
    // console.log('Delete ' + taskId);
    delete this._eventheap[taskId]; 
  },

  getTaskById: function(taskId){
    return this._eventheap[taskId]; 
  },
  ...
  exportState: function(timeState){
...
Hero.prototype.hpDec = function(){
  ...
  // this.timer.runOnce(self.destroy, self);
...
var Flame = function(c){
  ...
  this.timer = BM.game.timer;
}

Flame.prototype.start = function(){
  var self = this;

  this.timer.runOnce(self.destroy, self, 300);
...
Bomb.prototype.start = function (){
  ...
  this.timer.runOnce(self.explode, self, this.timeLeft);
...
Bomb.prototype.explode = function() {
  ...
  this.timer.runOnce(self.destroy, self, 1000);
